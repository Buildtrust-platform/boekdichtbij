...
function BookingSummaryStrict({ booking }: { booking: BookingData }) {
  return (
...
function BookingSummaryFlexible({ booking }: { booking: BookingData }) {
...
function SuccessContent({
  ...
  const isFlexible = serviceSlug === "dameskapper";
  const backUrl = `/${citySlug}/${areaSlug}/${serviceSlug}`;
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      try {
        ...
        if (data.status === "UNFILLED" || data.status === "DECLINED") {
          setState("unfilled");
          if (intervalRef.current) {
            clearInterval(intervalRef.current);
            intervalRef.current = null;
  ...
  const BookingSummary = isFlexible ? BookingSummaryFlexible : BookingSummaryStrict;
  const entityName = isFlexible ? "aanvraag" : "boeking";
  const entityNameCap = isFlexible ? "Aanvraag" : "Boeking";
      ...
      <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center">
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">
            {entityNameCap} niet gevonden
          </h1>
          <p className="text-gray-500 mb-6">
            Deze {entityName} bestaat niet of is verlopen.
          </p>
          <Link
            href={backUrl}
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center">
          <div className="w-12 h-12 border-2 border-gray-300 border-t-gray-600 rounded-full mx-auto mb-6 animate-spin" />
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">
            {isFlexible ? "Aanvraag verstuurd" : "Beschikbaarheid controleren"}
          </h1>
          <p className="text-gray-500 mb-1">
            We zoeken een beschikbare kapper{isFlexible ? " voor je" : ""}.
          </p>
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center">
          ...
          <h1 className="text-2xl font-semibold text-gray-900 mb-2">
            {isFlexible ? "Aanvraag geaccepteerd" : "Boeking bevestigd"}
          </h1>
          <p className="text-gray-500">
            {isFlexible
      ...
      <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center">
          ...
          <div className="space-y-3">
            ...
            >
              {isFlexible ? "Nieuwe aanvraag indienen" : "Kies een andere tijd"}
            </Link>
          </div>
        </div>
      </main>
